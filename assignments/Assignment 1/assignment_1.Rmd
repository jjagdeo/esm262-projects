...
waterstorage = function(precip, evap, runoff, watershed_size) {
  storage = (precip - (evap + runoff))*watershed_size
...
```{r}
#Generate sample data to test funtion
nsample = 100

#Data created for a 10 sqmile watershed
...
results = data.frame(precip = precip, evap = evap, runoff = runoff, storage = NA)

```

#### Test waterstorage() function with sample data and plot
```{r}
#Testing function for sample data and 10 sq mile watershed
results$storage = waterstorage(results$precip, results$evap,results$runoff, watershed_size = 10)
